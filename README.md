...
> FnGuide 1,054종목·311개월 패널을 ETL해 25개 Size×BM 포트폴리오를 만들고, 3-Factor 모델 적합성과 팩터 프리미엄을 미국(CRSP)과 비교 검증한 실증 연구.

## ⭐ 핵심 발견

- **HML(가치 팩터) 월 0.83%, t=3.89** — 한국 시장에서 가장 강한 프리미엄. 미국(0.41%)의 약 2배.
- **SMB 부호 반전 발견: 월 -0.67%(t=-2.57)** — 미국(+0.29%)과 반대. 소형주가 대형주를 이기지 못하는 구조.
- **GRS 검정 F=1.697 (p=0.023)** — 25개 포트폴리오에 대한 3-Factor 모델의 설명력과 한계를 통계적으로 제시.
- **FnGuide 원데이터 ETL부터 GRS까지 전체 파이프라인 재현** — 1,054개 종목, 2000-07~2026-05, 311개월.

## 주요 시각화

**미국 vs 한국 팩터 프리미엄 비교** — SMB 부호 반전이 한눈에 드러나는 결과물.

![미국 vs 한국 팩터 프리미엄](output/charts/08_us_vs_korea_comparison.png)

**25개 Size×BM 포트폴리오 평균 초과수익률 히트맵** — 고BM으로 갈수록 수익률이 체계적으로 높아지는 가치 효과.

![25 포트폴리오 히트맵](output/charts/03_heatmap_25_portfolios.png)

**팩터 누적수익률 (2000-2026)** — HML만 꾸준히 우상향, SMB는 금융위기 이후 하락세로 음의 누적수익률에 수렴.
...
25개 포트폴리오 3-Factor 회귀 평균 R²는 0.72(0.40~0.88). 시장 베타는 모두 1 부근에 분포하고, HML 베타는 BM 분위가 높을수록 0.59~1.13으로 단조 증가해 포트폴리오가 팩터 노출을 제대로 포착했음을 보여준다.

## SMB 부호 반전, 왜 일어났나

미국에서 규모 효과(Small Minus Big)는 양(+)이지만 한국은 반대다. 분석 기간 내내 소형주보다 대형주가 나은 성과를 냈다. 원인으로 다음을 짚었다.

- **시장 집중도**: 삼성전자·SK하이닉스·현대차 등 소수 대형 제조업체가 시가총액의 압도적 비중을 차지하고, 외국인·연기금 자금이 이들로 몰린다.
- **KOSDAQ의 성장주 편중**: 소형주 다수가 기술·바이오·게임 성장주다. 전통적인 가치형 소형주 프리미엄이 희석되거나 역전됐다.
- **Mkt-RF와의 음의 상관(-0.378)**: 시장 상승기에는 늘 대형주가 앞서는 구조가 팩터 수익률에 그대로 묻어난다.

## 방법론

Fama-French(1993) 원론을 최대한 준수하면서 한국 시장에 맞게 재구축했다.
...
- **표본**: KOSPI+KOSDAQ 1,054종목, 우선주·금융업 제외, 분석 기간은 IMF 외환위기 이후 한국 자본시장의 구조 변화(IT 버블, 글로벌 금융위기, COVID-19, 금리 인상기)를 모두 포함.
