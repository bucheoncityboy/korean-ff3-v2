# fama-french-korea-factor — 한국 Fama-French 3-Factor 모델 v2

> **One-Line Pitch:** FnGuide 311개월 패널 데이터 ETL부터 25개 Size-BM 포트폴리오 GRS 검정까지, 한국 시장 팩터 프리미엄 실증을 엔드투엔드로 재현

## 📌 Executive Summary

| Category | Details |
| :--- | :--- |
| **Core Objective** | Fama-French(1993) 3-Factor 모델을 한국 시장에 엄격하게 재구축 — 데이터 정제부터 팩터 추정·GRS 검정까지 전체 파이프라인 오류 없이 재현 |
| **Key Architecture** | FnGuide raw 데이터 패널 ETL(build_panel_data) → 결측치 보정·QA(qa_check) → 우선주/금융업 필터링 → 가치가중 25개 Size×BM 포트폴리오 → 3-Factor 회귀 → GRS 검정 → 미국(CRSP) 비교 |
| **Performance** | **1,054개 종목 · 311개월(2000-07~2026-05)** · GRS F=1.697 (p=0.023) · 25개 포트폴리오 평균 R² 0.72 · SMB 음의 프리미엄(-0.674%/월, t=-2.57) — **미국(+0.29%)과 부호 반전** 발견 |
| **Tech Stack** | Python, pandas, statsmodels, openpyxl, FnGuide 수정주가 데이터(총수익률), CRSP 벤치마크 |

---

## 핵심 발견 요약

| 팩터 | 미국 (CRSP) | 한국 (FnGuide) | 비교 |
|------|------------|----------------|------|
| Mkt-RF | 0.42%/월 | 0.71%/월 | 한국 시장 프리미엄 ↑ |
| **SMB** | **+0.29%/월** | **-0.674%/월 (t=-2.57)** | **부호 반대 — 한국은 대형주 우위** |
| HML | 0.41%/월 | 0.826%/월 (t=3.89) | 한국 가치 효과 2배 |

> **SMB 부호 반전 해석:** 한국 시장의 음의 규모 효과(Small -0.674%/월)는 대형주 집중 현상·KOSDAQ 구조 변화를 반영. Mkt-RF와 SMB의 음의 상관(-0.378)은 시장 상승기 대형주 아웃퍼폼 특성과 일치.
...
| 항목 | 내용 |
|------|------|
| **종목 수익률** | FnGuide (1,054개 종목, 2000-2026) |
| **무위험수익률** | FnGuide (월별) |
...
- **해석**: 포트폴리오 구성이 규모 노출도를 잘 포착하고 있음.
...
| 유동성 할인 | 한국 소형주는 미국보다 유동성 현저히 낮음. 기관 투자자 접근 어려움 |
